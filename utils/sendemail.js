      ...
      async function (err, html) {
        ...
        if (subject == "Store Created") {
          return res.status(200).send({
            status: 200,
            message: `Store created successfully and store detail is sent in email.`,
          });
          ...
          return;
        } else if (templates == "order-success") {
          console.log("order-success email")
          return true;
          ...
          return res.status(200).send({ success : true , message: `Store created successfully and store detail is sent in email.` });
        }
      }
    );
  } catch (error) {
